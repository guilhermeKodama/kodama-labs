...
export default function configureOpenAPI(app: OpenAPIHono<AppBindings>) {
  app.doc("/doc", {
    ...
    info: {
      ...
      title: "Sentinel API",
      description:
        "Government corruption tracker - Brazilian public procurement analysis",
    },
  });
  ...
  app.get(
    ...
    apiReference({
      ...
      metaData: {
        ...
        description: "API documentation for Sentinel corruption tracker",
      },
      defaultHttpClient: {
        targetKey: "js",
        clientKey: "fetch",
  ...
  app.get("/", (c) => {
    ...
    if (accept && accept.includes("text/html")) {
      return c.html(`
      <html>
        <head><title>Sentinel API</title></head>
        <body style="font-family: system-ui, sans-serif; max-width: 600px; margin: 50px auto; padding: 20px;">
          <h1>Sentinel API</h1>
          <p>Government corruption tracker - Brazilian public procurement analysis</p>
          <ul>
