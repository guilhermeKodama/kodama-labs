...
import { deleteCreditCardService } from "../../services/delete-credit-card";
...
export const route = createRoute({
  ...
  description: "Deletes a credit card for the authenticated user",
  request: {
    params: z.object({ id: z.string() }),
  },
  responses: {
    [OK]: jsonContent(SuccessResponseSchema, "Credit card deleted"),
...
export const handler: AppRouteHandler<typeof route> = async (c) => {
  try {
    ...
    await deleteCreditCardService(userId, id, prisma);

    return c.json({ message: "Credit card deleted successfully" }, OK);
  } catch (error: unknown) {
