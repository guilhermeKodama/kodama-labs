...
9. Monte o payload de `propose_import_plan`: `transactions` para o que vai virar `Transaction`, `transfers`/`investmentTransfers` para o que foi classificado como transferência, `reconciliations` para os `changed` que você decidiu atualizar, `duplicateDecisions` para os `fuzzy_match` já respondidos pelo usuário nesta conversa.
