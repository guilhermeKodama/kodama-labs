...
function RowActions({ rt, onEdit, onDelete, onToggle, onMarkPaid, onAttach, isMarkingPaid }: RowActionsProps) {
        ...
        {recurringTransfers.map((rt) => {
          ...
          return (
            <MobileListItem
              ...
              leading={
                <span className={cn('flex size-9 items-center justify-center rounded-lg', config.chipClassName)}>
                  <ArrowRight className="h-4 w-4" />
                </span>
              }
              title={rt.description || tDirections(config.label)}
              titleExtra={
                <AttachmentBadge
              ...
              trailing={<span className="text-purple-400">{formatCurrency(rt.amount * rt.exchangeRate, settings.baseCurrency)}</span>}
              trailingSub={
                <span className={cn(isOverdue && 'text-red-400', isDueToday && 'text-amber-400')}>
